...
import com.breakingthebasics.MainActivity;
...
import com.breakingthebasics.model.Gson_Sociallogin;
import com.breakingthebasics.model.Gson_login;
...
//                            finish();
    ...
    private String checkvalidation() {
        ...
        if (strpass.isEmpty() || strpass.length() < 3) {
...
//            accessToken = loginResult.getAccessToken().getToken();
    ...
    public class sociallogconn extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            super.onPostExecute(aVoid);
            service.getSocialLogin(fb_name, fb_gmail, "Facebook", strtoken, "0").enqueue(new Callback<String>() {
                @Override
                public void onResponse(Call<String> call, Response<String> response) {
                    String myres = response.body().toString();
                    myProgress.hideLoading();
